/**
 * Production deploy script — TeamVesting1 only (1.5M CASH allocation).
 *
 ...
 *   2. token.approve(vesting, 1.5M)
 *   3. vesting.depositToPool()
 *   4. Etherscan verification
 *
 * Usage:
 *   npx hardhat run scripts/deployVesting1Mainnet.js --network arbitrum
 */
...
const TEAM_ALLOC        = ethers.parseEther("1500000"); // 1.5 M CASH

const EXPECTED_DEPLOYER = "0x8DF2c30D46c3309685F1e5961E6001A1B8F818Dc";
...
async function main() {
  ...
  console.log("══════════════════════════════════════════════════════════════");
  console.log("  TeamVesting1 — Production Deploy (1.5M CASH)");
  console.log("══════════════════════════════════════════════════════════════");
  ...
  console.log("\n── Step 2 / 4 — token.approve(vesting, 1.5M)  [owner] ──────");
  const approveTx = await token.connect(deployer).approve(vesting.target, TEAM_ALLOC);
  await approveTx.wait();
  console.log("  tx :", approveTx.hash);

  // ── Step 3 / 4 — Owner calls depositToPool ───────────────────────────────
  console.log("\n── Step 3 / 4 — vesting.depositToPool()  [owner] ───────────");
  const vestingContract = await ethers.getContractAt("TeamVesting", vesting.target);
  ...
  console.log("  Total alloc    :", ethers.formatEther(await vesting.totalAllocation()), "CASH");
  ...
  console.log("    1. vesting.withdraw()                 — after cliff elapses (3 years)");
  console.log("    2. (wait 7-day Claiming cooldown)");
  console.log("    3. vesting.claimFromClaiming(<index>) — check index via The Graph");
  console.log("    4. vesting.claim()                    — call monthly to receive CASH");
  console.log("══════════════════════════════════════════════════════════════");
}
